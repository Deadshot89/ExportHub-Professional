...
test('Professional 0.6 normalizes customer locations and resolves shipment location',async()=>{
  const payload=typedSample(false);
  ...
  const pkg=await buildMigrationPackage(payload,JSON.stringify(payload));
  assert.equal(pkg.normalized.schemaVersion,'professional-0.6');
...
test('Professional 0.6 structures audit and redacts secret keys',async()=>{
  const payload=typedSample(false);
...
test('Professional 0.6 assigns recovery actions without pretending missing source files exist',async()=>{
...
test('Professional 0.6 preserves generated document metadata separately from migrated files',async()=>{
  const payload=typedSample(false);
