...
test('Professional 0.7 normalizes customer locations and resolves shipment location',async()=>{
  const payload=typedSample(false);
  ...
  const pkg=await buildMigrationPackage(payload,JSON.stringify(payload));
  assert.equal(pkg.normalized.schemaVersion,'professional-0.7');
...
test('Professional 0.7 structures audit and redacts secret keys',async()=>{
  const payload=typedSample(false);
...
test('Professional 0.7 assigns recovery actions without pretending missing source files exist',async()=>{
...
test('Professional 0.7 preserves generated document metadata separately from migrated files',async()=>{
  const payload=typedSample(false);
