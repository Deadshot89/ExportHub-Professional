...
test('masterdata schema bootstrap is explicit, serialized, idempotent and tenant-safe',async()=>{
...
test('schema bootstrap failure has deterministic HTTP mapping',()=>{
  const src=fs.readFileSync(new URL('../api/shared/http.js',import.meta.url),'utf8');
  assert.match(src,/MASTERDATA_SCHEMA_UPGRADE_DISABLED:503/);
});
