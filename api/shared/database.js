...
function status(){ return {configured:configured(),dataMode:DATA_MODE,writesEnabled:writesEnabled()}; }

function getPool(){
  if(!configured()) throw Object.assign(new Error('Professional database is not configured.'),{code:'DATABASE_NOT_CONFIGURED'});
  if(!pool){
    const {Pool}=require('pg');
    pool=new Pool({connectionString:process.env.PROFESSIONAL_DATABASE_URL,max:Number(process.env.PROFESSIONAL_DB_POOL_MAX||5),ssl:process.env.PROFESSIONAL_DATABASE_SSL==='false'?false:{rejectUnauthorized:true}});
  }
  return pool;
}
...
async function withTenantClient(tenantId,fn,{write=false}={}){
  ...
  try{
    ...
    if(write) await client.query('COMMIT'); else await client.query('ROLLBACK');
    return result;
  }catch(err){
...
module.exports={configured,writesEnabled,status,withTenantClient};
