...
function ensureCommercialStory() {
  const existing = document.getElementById('commercialStoryPanel');
  if (existing) return existing;
...
export function initPresentationGuide({ openView, openShipment, setPresentationRole } = {}) {
  ...
  const startButtons = [document.getElementById('startTourBtn'), document.getElementById('restartTourBtn'), document.getElementById('commercialStartTourBtn')].filter(Boolean);
  let index = 0;
